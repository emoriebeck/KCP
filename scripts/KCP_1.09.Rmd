...
KCP_nested <- weekly_wide %>% 
  ...
  , perm_data = sprintf("%s/results/perm_data", write.path) # path to save the permuted data sets and moving window correlations to
  , vhat_data = sprintf("%s/results/vhats", write.path) # path to save the v hat values, if desired
  , ID = .y  
  ), NA_real_)))
```
