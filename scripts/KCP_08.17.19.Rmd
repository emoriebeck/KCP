...
```{r}
weekly_dem <- haven::read_sav("~/Downloads/metaresearch2-initial-web.sav")
...
load_fun <- function(sid){
  ...
  load(sprintf("%s/out/results_%s.RData", "~/Box/network/other projects/KCP/results", sid))
...
files <-  "~/Box/network/other projects/KCP/results/out" %>% list.files() %>% 
# files <-  sprintf("%s/blob/master/results/out?raw=true", read.path) %>% list.files() %>% 
...
```{r}
load(url(sprintf("%s/blob/master/results/KCP.RData?raw=true", read.path)))
